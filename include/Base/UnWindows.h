 ...
 * or not. Technosoftware always calls the Unicode or ASCII functions directly (depending
 * on whether TECHNOSOFTWARE_WIN32_UTF8 is #define'd or not), so the macros are not ignored.
 *
 ...
 * GetUserName. Depending on whether "Base/UnWindows.h" has been included by a particular
 * translation unit or not, this might be changed to GetUserNameA/GetUserNameW, or not.
 * While, due to naming conventions used, this is less of a problem in Technosoftware, some
 * of the users of Technosoftware might use a different naming convention where this can become
 * a problem.
 *
 * To disable the #undef's, compile Technosoftware with the TECHNOSOFTWARE_NO_UNWINDOWS macro #define'd.
 *
...
#ifndef TECHNOSOFTWARE_UNWINDOWS_H
#define TECHNOSOFTWARE_UNWINDOWS_H


// Reduce bloat
#if defined(_WIN32)
    #if !defined(WIN32_LEAN_AND_MEAN) && !defined(TECHNOSOFTWARE_BLOATED_WIN32)
...
// definitions.) For more information, see SdkDdkVer.h.
    ...
    // If allowed (see TECHNOSOFTWARE_MIN_WINDOWS_OS_SUPPORT
...
// #define TECHNOSOFTWARE_FORCE_MIN_WINDOWS_OS_SUPPORT


#include <windows.h>


#if !defined(TECHNOSOFTWARE_NO_UNWINDOWS)
...
#undef GetVersion
#undef GetObject
#endif // TECHNOSOFTWARE_NO_UNWINDOWS

#endif /* TECHNOSOFTWARE_UNWINDOWS_H */
